...
self-attention. For an input feature ([]{#_Hlk223879809
.anchor}$X \in \mathbb{R}^{B \times H \times W \times C}$), the window
partition operation produces
($X_{w} \in \mathbb{R}^{(B \cdot N_{w}) \times N \times C}$), where
($N = w^{2}$) is the number of tokens in each window. A linear
projection is then used to generate four branches, namely query, key,
value, and mask features:

$$\lbrack Q,K,V,M\rbrack = Linear(X_{w}).$$

$$S = QK^{\top} + B_{\text{rel}} + M_{\text{sw}},$$

where ($B_{\text{rel}}$) denotes the learnable relative position bias
and ($M_{\text{sw}}$) is the shifted-window mask when window shifting is
enabled.

Different from standard window attention, DMMA introduces a
difference-aware gating mechanism derived from the mask branch M, so
that local inconsistency between token pairs can be explicitly modeled
during feature interaction. Specifically, pairwise difference cues are
constructed from the mask features to measure the local inconsistency
between token pairs. After normalization, the difference response is
converted into a suppressive gate:

$$G(\Delta) = 1 - \sigma(\Delta),$$

where Δ denotes the normalized pairwise difference cue and σ(⋅) is the
sigmoid function. In this way, token pairs with highly inconsistent
local responses are assigned smaller gating weights, while structurally
consistent interactions are preserved.
...
where τ is a learnable temperature parameter. The learnable temperature
avoids manually fixing the strength of difference-aware suppression and
allows the model to adapt the gating intensity to different maritime
scenes and feature scales. The output feature of the attention branch is
then obtained by
