...
responses and reduce redundant sea-surface texture activations. Third,
comparison experiments are conducted on MASATI and HRSC2016-MS against
representative CNN-based, Transformer-based, and ship-specific
detectors, including YOLOv8, RT-DETR, DINO, and LiM-YOLO, and ablation
studies are further performed to analyze the individual and
complementary effects of DMMA and ECA. The proposed model achieves the
best mAP@0.5 on both datasets, demonstrating its effectiveness for
target discovery and target-background discrimination in optical remote
...
representation while maintaining practical inference speed.
...
hardware and inference settings, and the reported values are averaged
over three repeated runs. The baseline YOLOv12 model achieves 60 FPS,
while the ECA-only variant achieves 66 FPS. After introducing DMMA, the
FPS reaches 72, and the final YOLOv12 + DMMA + ECA model achieves 76
FPS. These results indicate that the proposed variants maintain
practical inference throughput on the RTX 4090 platform. Therefore, the
proposed YOLOv12 + DMMA + ECA model is suitable for offline or
near-real-time maritime surveillance, port monitoring, and remote
sensing interpretation scenarios where detection reliability and
...
remains a challenging direction for future improvement.

**Table 4.5 Inference speed comparison of different YOLOv12 variants on
HRSC2016-MS**
...
while maintaining practical inference speed. When combined with DMMA,
...
boundary-aware regression constraints, and localization-enhanced
detection heads will be explored to improve high-IoU detection quality
while maintaining strong target discovery performance.
