...
and inference efficiency. This trend establishes the theoretical basis
for integrating mixed attention structures into YOLO-style detectors for
small-ship tasks.
...
Since DINO does not directly report traditional Precision and Recall
values in our current evaluation record, only its mAP-based metrics are
listed in Tables 4.2 and 4.4. In particular, the Recall-related quantity
provided by DINO follows the COCO evaluation protocol and is reported as
AR@100 rather than the conventional Recall under a fixed confidence
threshold. Therefore, the traditional Precision and Recall values of
DINO are not reported here.
