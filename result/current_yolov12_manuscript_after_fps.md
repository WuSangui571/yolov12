...
to analyze the individual and complementary effects of DMMA and ECA..
The proposed model achieves the best mAP@0.5 on both datasets,
demonstrating its effectiveness for target discovery and
target-background discrimination in optical remote sensing ship
detection.
...
results indicate that lightweight channel recalibration alone can
improve feature selection and suppress redundant background responses to
a certain extent. However, the improvement brought by ECA alone is still
limited compared with the DMMA-only model. After introducing DMMA,
Precision increases to 0.834, Recall to 0.728, mAP@0.5 to 0.791, and
mAP@0.5:0.95 to 0.522, demonstrating that DMMA is the dominant
contributor to target-background discrimination and small-ship
representation enhancement under complex maritime backgrounds.
...
and mAP@0.5:0.95 by 0.009. This suggests that ECA is more effective when
applied after difference-aware spatial-token interaction, because DMMA
first enhances target-background separability, while ECA then
recalibrates informative channels and suppresses residual
background-dominated responses. Therefore, the ablation results verify
the complementary roles of DMMA and ECA: DMMA provides the main
discrimination gain, and ECA further refines channel-wise feature
representation with negligible additional complexity.
...
In addition to accuracy, computational cost is an important
consideration for practical deployment. As shown in Table 4.5, the
baseline YOLOv12 model has 2.112 M parameters and 5.8 GFLOPs.
Introducing ECA alone does not increase the reported parameter count or
GFLOPs, indicating that the channel refinement branch is lightweight. In
contrast, introducing DMMA increases the parameter count from 2.112 M to
2.509 M and GFLOPs from 5.8 to 8.3, corresponding to an increase of
about 43% in computational cost. This indicates that the main
performance gain and the main additional computation are both primarily
associated with stronger difference-aware attention-based feature
interaction. The final YOLOv12 + DMMA + ECA model maintains the same
reported complexity as the DMMA-only variant while achieving higher
accuracy, suggesting that ECA provides complementary channel refinement
with negligible additional overhead. Therefore, the proposed YOLOv12 +
DMMA + ECA model should be understood as an accuracy-oriented
configuration for remote sensing image analysis rather than a highly
constrained real-time deployment model. The reason for retaining the
YOLOv12 framework is that it provides a mature, efficient, and easily
reproducible detection pipeline, while DMMA selectively enhances the
feature interaction stages that are most affected by maritime clutter.
This makes the method more suitable for offline or near-real-time
maritime surveillance, port monitoring, and remote sensing
interpretation scenarios where detection reliability is prioritized over
...
This indicates that DMMA and ECA mainly improve target discovery and
target-background discrimination, while stricter boundary localization
remains a challenging direction for future improvement.. This indicates
that future work should further improve high-IoU localization accuracy
while maintaining the target-background discrimination advantage brought
by DMMA.
...
height="2.7784722222222222in"} The experimental results indicate that
DMMA is the dominant contributor to target-background discrimination and
small-ship target discovery, while the ECA-only ablation confirms that
lightweight channel recalibration can also improve feature selection
with almost no additional reported complexity. When combined with DMMA,
ECA further strengthens channel-wise feature responses by suppressing
redundant maritime background activations. However, the comparison with
LiM-YOLO also shows that the proposed method still has room for
improvement under stricter localization metrics such as mAP@0.5:0.95. In
future work, we will further investigate more fine-grained
mechanism-level ablations, including attention variants without
difference-aware gating, different DMMA insertion positions, and
sensitivity analyses of the learnable temperature and mask scaling
factors. In addition, more efficient difference-aware attention designs,
boundary-aware regression constraints, localization-enhanced detection
heads, and unified latency evaluation will be explored to improve
high-IoU detection quality while maintaining strong target discovery
performance.
