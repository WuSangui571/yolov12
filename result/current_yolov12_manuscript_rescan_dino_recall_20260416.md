...
Precision and Recall values under a fixed confidence threshold, only its
mAP-based metrics are reported in Tables 4.2 and 4.4. Specifically, the
recall-related quantity available from DINO follows the official COCO
evaluation protocol and is reported as AR@100, which is not equivalent
to the conventional Recall used by the YOLO-style detectors in this
paper. To avoid mixing heterogeneous metric definitions within the same
table, the Precision and Recall entries of DINO are left blank.
...
compared methods, with a Recall of 0.682 and mAP@0.5 of 0.824. These
results indicate that the proposed design is particularly effective for
target discovery and moderate-IoU detection in complex maritime
backgrounds.
