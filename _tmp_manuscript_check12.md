...
|                                   | content-adaptive modulation of    |
|                                   | pairwise affinity before softmax. |
|                                   | Because the pre-softmax logit may |
|                                   | be positive or negative, this     |
|                                   | formulation is not intended to    |
|                                   | claim strict pointwise            |
|                                   | suppression for every sign        |
|                                   | condition. Instead, it is used to |
|                                   | relatively weaken structurally    |
|                                   | inconsistent interactions on      |
|                                   | average during attention          |
|                                   | normalization while preserving    |
|                                   | the original attention form and   |
|                                   | implementation simplicity. A      |
|                                   | direct comparison with additive   |
|                                   | bias based formulations remains   |
...
|                                   | mAP@0.5:0.95. For validation      |
|                                   | under the Ultralytics detection   |
|                                   | pipeline, the confidence          |
|                                   | threshold followed the default    |
|                                   | val setting of 0.001 and the NMS  |
|                                   | IoU threshold was 0.7. Unless     |
|                                   | otherwise stated, the comparisons |
|                                   | within the YOLOv12 family used    |
|                                   | the same optimizer, training      |
|                                   | schedule, input resolution, and   |
...
|                                   | configurations reproduced in this |
|                                   | study were YOLOv8x, RT-DETR-v2,   |
|                                   | DINO-4scale with a ResNet-50      |
|                                   | backbone, LiM-YOLO, and           |
|                                   | FBVF-YOLO. Unless prevented by    |
...
|                                   | the HBB results reported in this  |
|                                   | study should not be directly      |
|                                   | compared with                     |
